...
let
  ...
  pkgs = import <nixpkgs> { config.android_sdk.accept_license = true; config.allowUnfreePredicate = pkg: builtins.elem (lib.getName pkg) [ "android_sdk" "libsciter" ]; config.permittedInsecurePackages = [ "olm-3.2.16" "mbedtls-2.28.10" "gradle-7.6.6" "qtwebengine-5.15.19" ]; };
